...
char val[1024] ;
static long my_ioctl(struct file *file, unsigned int cmd, unsigned long arg) ; // static struct file_operations fops : .unlocked_ioctl
...
static long my_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	...
	switch(cmd) {
		case WR_DATA:
			res=copy_from_user(&val, (char*)arg, sizeof(val)) ;
			printk(KERN_INFO"[INFO] val = %s <-> return from copy_to_user : %d\n", val, res) ;
			break ;
		case RD_DATA:
			res=copy_to_user((char*)arg, &val, sizeof(val)) ;
			printk(KERN_INFO"[INFO] Data read : DONE <-> return from copy_to_user : %d\n", res ) ;
			break ;
	}
	return 0 ;
